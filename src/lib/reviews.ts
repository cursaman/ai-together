...
export async function getPublishedReviews(limit?: number): Promise<PublicReview[]> {
  ...
  let query = (await createClient()).from("ait_reviews").select("id,nickname,content,result_image_url,created_at,ait_meetings(title)").eq("is_published", true).order("created_at", { ascending:false });
