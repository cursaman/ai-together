...
export default async function Home() {
  ...
  const oneDayMeeting = openMeetings.find((meeting) => getCourseType(meeting.title) === "1일 체험");
  const fourWeekMeeting = openMeetings.find((meeting) => getCourseType(meeting.title) === "4주 완성");
  const oneDayApplyHref = oneDayMeeting ? `/meetings/${oneDayMeeting.id}/apply` : oneDayHref;
  const fourWeekApplyHref = fourWeekMeeting ? `/meetings/${fourWeekMeeting.id}/apply` : fourWeekHref;
  ...
  return <>
    ...
    <main id="main-content">
      <section className="market-hero conversion-hero" aria-labelledby="hero-title">
        ...
        <div className="market-hero-copy"><p>완전 초보를 위한 소규모 AI 실습</p><h1 id="hero-title">AI로 만들고,<br />내 것으로 완성해요.</h1><span>AI로 사진도 만들고, 홈페이지도 만들고, 나만의 작은 서비스를 직접 만들어봅니다.</span><strong className="hero-beginner">코딩 몰라도 OK · 노트북만 준비</strong></div>
        <div className="hero-choice-card" aria-label="과정 바로 신청"><Link href={oneDayApplyHref}><span>가볍게 경험하기</span><strong>1일 체험 신청</strong><small>15시–17시 · 2시간 · 10,000원</small></Link><Link href={fourWeekApplyHref}><span>내 결과물 완성하기</span><strong>4주 과정 신청</strong><small>18시–20시 · 총 8시간 · 80,000원</small></Link></div>
      </section>
      ...
      {reviews.length > 0 ? <section className="home-reviews trust-reviews" aria-labelledby="home-reviews-title"><div className="market-heading"><div><p>REAL REVIEWS</p><h2 id="home-reviews-title">참여자가 직접 들려준 이야기</h2><span>공개에 동의한 실제 참여 후기만 소개합니다.</span></div><Link href="/reviews">전체 후기 보기 →</Link></div><div className="home-review-grid">{reviews.map((review)=><article key={review.id}>{review.result_image_url?<div className="home-review-image" role="img" aria-label={`${review.nickname}님의 결과물`} style={{backgroundImage:`url(${review.result_image_url})`}}/>:<span className="home-review-quote" aria-hidden="true">“</span>}<div><small>{review.ait_meetings?.title??"AI Together 모임"}</small><p>{review.content}</p><strong>{review.nickname}</strong></div></article>)}</div></section>:null}

      <section className="course-compare" aria-labelledby="compare-title"><div className="market-heading"><div><p>CHOOSE YOUR COURSE</p><h2 id="compare-title">목표에 맞게 선택하세요</h2><span>두 시간 체험부터 인터넷에 공개하는 완성 과정까지 준비했습니다.</span></div></div><div className="compare-grid"><article><div className="compare-card-heading"><span>가볍게 시작</span><h3>1일 체험</h3><strong>10,000원</strong></div><dl><div><dt>추천 대상</dt><dd>AI를 처음 경험해보고 싶은 분</dd></div><div><dt>수업 시간</dt><dd>1회 2시간 · 15시–17시</dd></div><div><dt>완성 결과</dt><dd>이미지·영상·작은 홈페이지</dd></div></dl><Link href={oneDayApplyHref}>1일 체험 신청하기 →</Link></article><article className="compare-featured"><div className="compare-card-heading"><span>하나를 제대로 완성</span><h3>4주 완성</h3><strong>80,000원</strong></div><dl><div><dt>추천 대상</dt><dd>내 홈페이지나 유튜브를 만들고 싶은 분</dd></div><div><dt>수업 시간</dt><dd>주 1회 2시간 · 총 8시간</dd></div><div><dt>완성 결과</dt><dd>인터넷에 공개하는 나만의 결과물</dd></div></dl><Link href={fourWeekApplyHref}>4주 과정 신청하기 →</Link></article></div></section>
      ...
      <section className="visit-section" aria-labelledby="visit-title"><div><p>LOCATION & PREPARATION</p><h2 id="visit-title">커피와 빵은 준비할게요.<br />노트북만 챙겨오세요.</h2><ul><li><strong>장소</strong><span>부산 교대 인근 커피긱스 2층 8인룸</span></li><li><strong>카페 이용</strong><span>커피·빵 50,000원은 진행자가 참가비에서 일괄 결제</span></li><li><strong>추가 결제</strong><span>참가자는 별도 공간 이용료와 음료 결제 없음</span></li><li><strong>준비물</strong><span>개인 노트북과 충전기</span></li></ul><Link className="text-link" href="/contact">장소와 이용 안내 자세히 보기 →</Link></div><figure className="visit-menu"><Image src="/images/coffee-geeks-menu.jpg" alt="커피긱스 커피와 음료 메뉴판" width={900} height={742} sizes="(max-width: 900px) calc(100vw - 40px), 44vw" /><figcaption><strong>커피긱스 메뉴</strong><span>메뉴와 가격은 방문 시점에 따라 달라질 수 있습니다.</span></figcaption></figure></section>
      <section className="final-cta" aria-labelledby="final-cta-title"><p>READY TO MAKE?</p><h2 id="final-cta-title">이번에는 무엇을<br />직접 만들어볼까요?</h2><span>AI가 처음이어도 괜찮습니다. 원하는 과정과 날짜를 고르면 바로 시작할 수 있어요.</span><div><Link href={oneDayApplyHref}>1일 체험 · 10,000원</Link><Link href={fourWeekApplyHref}>4주 완성 · 80,000원</Link></div></section>
    </main>
    <div className="mobile-apply-bar"><div><small>처음이라면 가볍게</small><strong>1일 체험 · 10,000원</strong></div><Link href={oneDayApplyHref}>모임 신청</Link></div>
