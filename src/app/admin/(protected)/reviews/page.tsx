...
export default async function AdminReviewsPage({searchParams}:{searchParams:Promise<{created?:string;error?:string}>}) {
  const params=await searchParams;
  const [reviews,meetings]=await Promise.all([getAdminReviews(),getAdminMeetings()]);
  const notice=params.created?"후기를 등록했습니다.":params.error==="invalid"?"닉네임과 후기 글자 수를 확인해주세요.":params.error?"후기를 저장하지 못했습니다.":null;
